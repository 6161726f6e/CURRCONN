...
touch netput.txt
touch netstat.tmp
netstat -antup | awk '{print $4, $5, $6, $7}' | grep '^[1-9]' > netstat.tmp
#cat netstat.tmp

while read myline; do
	IP=`echo $myline | awk '{print $2}' | cut -d":" -f1`
	IPP=`echo $myline | awk '{print $2}'`
	DPORT=`echo $myline | awk '{print $2}' | cut -d":" -f2`
	LPORT=`echo $myline | awk '{print $1}' | cut -d":" -f2`
